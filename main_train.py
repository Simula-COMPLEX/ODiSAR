...
logger.info("Computing reconstruction error and forecast uncertainty thresholds...")
X_val, y_val = create_sequences(val_inputs, val_outputs, input_window, forecast_horizon, overlap=False)

val_recon_errors = compute_forecast_recon_errors(model, (X_val, y_val),
                                                 TRAINING_PARAMS['batch_size'], device)
recon_threshold = val_recon_errors.mean() + 3 * val_recon_errors.std()
val_variances = compute_mc_forecast_variance(model, (X_val, y_val),
                                             TRAINING_PARAMS['batch_size'], device, n_passes=20)
...
logger.info(f"Reconstruction threshold: {recon_threshold:.6f}")
logger.info(f"Forecast variance threshold: {variance_threshold:.6f}")

# === Forecast Evaluation ===
logger.info("Evaluating forecast performance on training set...")
...
os.makedirs(os.path.dirname(OUTPUT_PATHS['forecast_results_csv']), exist_ok=True)
...
logger.info(f"Forecast plot saved to: {OUTPUT_PATHS['forecast_plot_path']}")
